...
public class OciStartCommand implements Callable<Integer> {

    @Mixin
    OciGlobalOptions global;

    @Option(names = {"--port"}, description = "Host port to bind (default: 4599)", defaultValue = "4599", paramLabel = "<port>")
    int port;
    ...
    @Option(names = {"--image"}, description = "Image reference to use (default: floci/floci-oci:latest)", defaultValue = "floci/floci-oci:latest", paramLabel = "<ref>")
    String image;
    ...
    public Integer call() {
        ...
        try {
            ...
            if (existing.isPresent()) {
                ...
                if ("running".equals(state)) {
                    printer.error("Container '" + global.container + "' is already running.\nRun 'floci oci stop' first or pass --container <name> to use a different name.");
                    return 1;
                }
        ...
        if (persistDir != null) {
            ...
            args.addAll(List.of("-e", "FLOCI_OCI_STORAGE_MODE=persistent"));
        }
        if (services != null && !services.isBlank()) {
            args.addAll(List.of("-e", "FLOCI_OCI_SERVICES=" + services));
        }
        args.add(image);

        try {
            printer.println("Starting " + Ansi.gold("Floci OCI") + " container...");
        ...
        if (detach) {
            printer.println(Ansi.gray("Detached. Run 'floci oci wait' to poll for readiness."));
            return 0;
        }

        global.endpoint = "http://localhost:" + port;

        printer.println(Ansi.gray("Waiting for Floci OCI to be ready..."));
        OciWaitCommand wait = new OciWaitCommand();
        wait.global = global;
        wait.timeout = "30s";
        return wait.call();
    }
