...
- `CHANGELOG.md` is generated by semantic-release from Conventional Commit messages — **never edit it by hand**. The commit message *is* the changelog entry, so write it from the user's point of view (what was broken or what is new, not how it was implemented). `changelog-guard.yml` fails any PR that touches the file; genuine corrections to history need the `changelog-edit` label.
